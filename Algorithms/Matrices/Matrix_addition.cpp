// Matrix Addition in simple way with static, const, 2D-vector, functions and function prototype.
...
void matrix_addition(const vector<vector<int>> &matrix1, const vector<vector<int>> &matrix2, vector<vector<int>> &matrixSum);
...
void matrix_addition(const vector<vector<int>> &matrix1, const vector<vector<int>> &matrix2, vector<vector<int>> &matrixSum)
{
    for (size_t i{}; i < matrixSum.size(); ++i)
    {
        for (size_t j{}; j < matrixSum[i].size(); ++j)
        {
            matrixSum.at(i).at(j) = matrix1.at(i).at(j) + matrix2.at(i).at(j);
        }
    }
}
...
int main()
{
    ...
    if (matrix1Row == matrix2Row && matrix1Column == matrix2Column)
    {
        ...
        vector<vector<int>> matrixSum(matrix1Row, vector<int>(matrix2Column));

        // Perform Matrix-1 and Matrix-2 addition and store it to MatrixSum
        matrix_addition(matrix1, matrix2, matrixSum);
        // Dsiplay MatrixSum values
        display_matrix(matrixSum);
    }
    else
    {
        cout << "Matrix Addition is not possible due to different order." << endl;
    }
